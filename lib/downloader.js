...
function fetchLatestReleaseVersion() {
  return new Promise((resolve) => {
    ...
    req.on('error', () => resolve(DEFAULT_VERSION));
...
async function getRequestedVersion(rawArgs = process.argv.slice(2)) {
  for (let i = 0; i < rawArgs.length; i += 1) {
    const arg = rawArgs[i];
    if ((arg === '--pbl-version' || arg === '--pb-version') && rawArgs[i + 1]) {
      return rawArgs[i + 1].replace(/^v/, '');
    }
  }

  if (process.env.POCKETBASE_LIBSQL_VERSION) {
    return process.env.POCKETBASE_LIBSQL_VERSION.replace(/^v/, '');
  }
...
async function getBinaryInfo(rawArgs) {
  const version = await getRequestedVersion(rawArgs);
...
async function downloadBinary(rawArgs) {
  const info = await getBinaryInfo(rawArgs);
...
async function ensureBinary(rawArgs) {
  const info = await getBinaryInfo(rawArgs);
  ensureDir(info.versionDir);
